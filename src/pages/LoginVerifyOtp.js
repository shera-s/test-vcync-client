...
import { LOGIN_OTP } from "../config/gql/mutations";
// import { RESEND_OTP } from '../config/gql/mutation/ResendOtp';
...
const LoginVerifyOTP = () => {
  ...
  const [loginOtp, { data, loading, error }] = useMutation(LOGIN_OTP);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userdata = await loginOtp({ variables: { otp: otpData } });
      if (userdata) {
    ...
    <div className="bg-grey-lighter min-h-screen flex flex-col">
      <div className="container max-w-lg mx-auto flex-1 flex flex-col items-center justify-center px-2">
        <div className="bg-white px-6 py-8 rounded shadow-md text-black w-full">
          ...
          {/* <button onClick={resendOTP} className='w-full text-left font-medium text-xs hover:text-green-900'>Resend OTP</button> */}
